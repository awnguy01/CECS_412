...
.equ	ADCSRA,0x7A				;specifies the address offset of the ADC control and status register A
.equ	ADMUX,0x7C				;specifies the address offset of the ADC multiplexer selection register
.equ	ADCSRB,0x7B				;specifies the address offset of the ADC control and status register B
...
.equ	ADSC,6					;receives the ADC start conversion flag
.equ	ADIF,4					;receives the ADC interrupt flag
.equ	ADCL,0x78				;specifies the address of the ADC data register low
.equ	ADCH,0x79				;specifies the address of the ADC data register high
...
Mega328P_Init:
		...
		sts	ADCSRA,r16			;sets ADC Enable and the division factor between the system 
									;clock frequency and the input clock to the ADC to 128
		ldi r16,0x40			;sets the voltage reference selection
		sts ADMUX,r16			;voltage reference selection set to AVCC with 
									;external capacitor at AREF pin
		ldi r16,0			    ;sets the ADC trigger source
		sts ADCSRB,r16			;ADC trigger source set to free running mode
